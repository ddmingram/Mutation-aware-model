...
%%%%%%%%%%%%%%% CONSTRUCT C %%%%%%%%%%%%%%%
if dim_vec(x) == 1 % Promoter
    prom_plusC_vec = parts_combos(:,ActiveDim)';
    prom_minusC_vec = prom_plusC_vec;
    ActiveDim=ActiveDim+1;
else
    prom_plusC_vec = [ones(n,1)*parts_mat_all(1,x)]';
    prom_minusC_vec = prom_plusC_vec;
end
x=x+1;

if dim_vec(x) == 1 % RBS
    RBS_plusC_vec = parts_combos(:,ActiveDim)';
    RBS_minusC_vec = RBS_plusC_vec;
    ActiveDim=ActiveDim+1;
else
    RBS_plusC_vec = [ones(n,1)*parts_mat_all(1,x)]';
    RBS_minusC_vec = RBS_plusC_vec;
end
x=x+1;

if dim_vec(x) == 1 % CDS
    CDSC_vec = parts_combos(:,ActiveDim)' .* n_HC;
    
    noHmutC_idx = []; % For H_avg, need subpops that mutate CDS
    for i=1:length(CDSC_vec)
        if CDSC_vec(i) == CDSC_vec(1)
            noHmutC_idx = [noHmutC_idx,i];
        end
    end
else
    CDSC_vec = [ones(n,1) .* parts_mat_all(1,x) * n_HC]';
end