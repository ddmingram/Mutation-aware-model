...
%%% when expressing three synthetic genes (e.g. a toggle switch). For more
...
%%% toggle switch's inhibition and protein degradation tags.
K_H = sqrt(0.1);        % Half-saturation constant for H-repression (molecs/cell)
h_H = 2;                % Hill coefficient for H-repression
p_deg = 0;              % Protein degradation rate (/h)
K_I = 1;                % Half-saturation constant for inhibition (molecs/cell)

%%% Consolidate into one variable
base_params = [N, mass, nut, nq, v_e, K_e, n_R, n_Z, n_Q, n_HA, n_HB,...
               v_TX_R, v_TX_Z, v_TX_Q, K_TX_R, K_TX_nR, K_Q, h_Q, m_deg,...
               kb_TL, ku_TL, v_TL, K_TL, K_H, h_H, p_deg, K_I];