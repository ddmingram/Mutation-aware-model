...
p_vec_x(1,:) = p;
m_vec_x(1,:) = m;
alpha_x = 1 - p_vec_x(1,end) / sum(p_vec_x(1,:)); % Burden boost
...
%     step   
    ...
    %%% Cell division
    lamEtoM = lam0 * ((E*z+M) * E/(E+M)) * (1+alpha_x); % E --> M
...
plot(t_vec, EM_vec(:,1), 'Color', E_yellow)
plot(t_vec, EM_vec(:,2), 'Color', M_orange)
% legend('E (Resistant seq.)', 'M (Resistant seq.)','E (Default)','M (Default)','Location','East','FontSize',12)
